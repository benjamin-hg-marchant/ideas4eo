...
from ..data_preparation import date_converter
from .. import config

##########################################################################################

def create_reperestory(product,collection,year,month,day):
    ...
    pkg_path = pkg_path + '/data/files'
    
    if not os.path.exists("{}/{}".format(pkg_path,product)):
        os.system("mkdir {}/{}".format(pkg_path,product))

    if not os.path.exists("{}/{}/{}".format(pkg_path,product,collection)):
        os.system("mkdir {}/{}/{}".format(pkg_path,product,collection))
        
    if not os.path.exists("{}/{}/{}/{}_{}_{}".format(pkg_path,product,collection,year,month,day)):
        os.system("mkdir {}/{}/{}/{}_{}_{}".format(pkg_path,product,collection,year,month,day))
...
def get_file(product,collection,year,month,day,hour,minute):
    ...
    if file_name != -1: 
        ...
        file_path = "{}/{}/{}/{}_{}_{}/{}".format(pkg_path,product,collection,year,month,day,file_name)
        
        if not path.exists(file_path):
            create_reperestory(product,collection,year,month,day)
